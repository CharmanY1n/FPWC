...
def traverse_tree(xml_path, elem_list, attrib, add_index=False):
    ...
    for event, elem in ET.iterparse(xml_path, ['start', 'end']):
        if event == 'start': 
            ...
            if (attrib in elem.attrib and elem.attrib[attrib] == "true") or ("resource-id" in elem.attrib and "linearLayout" in elem.attrib["resource-id"]):
                parent_prefix = ""
                if len(path) > 1: 
...
class AndroidController:
    ...
    def get_device_size(self):
        ...
        result = execute_adb(adb_command) #Physical size: 1080x2220
        if result != "ERROR":
    ...
    def tap(self, x, y):
        ...
        while True:
            try:
                ...
                count += 1
                if count > 10:
                    raise Exception("Failed to tap the screen")
